...
import { Trash2, Type, Video, Image as ImageIcon, Palette, MonitorPlay, Upload, RefreshCw, Zap, Plus, Layers } from 'lucide-react';
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
        ...
        setDraft(prev => ({
            ...prev,
            contentType: type,
            content: { ...prev.content, url: res.data.filePath }
        }));
    } catch (err) { toast.error('Upload failure'); }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 bg-black/80 rounded-[32px] border border-white/10 flex items-center justify-between group overflow-hidden">
                    <div className="relative z-10">
                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-1">Global Fallback</p>
                        <h4 className="text-2xl font-black text-white tabular-nums uppercase">{configs.filter(c => c.targetType === 'all').length} Protocols</h4>
                    </div>
                    <Zap className="text-emerald-500/20 group-hover:scale-125 transition-transform" size={48} />
                </div>
                <div className="p-6 bg-black/80 rounded-[32px] border border-white/10 flex items-center justify-between group overflow-hidden">
                    <div className="relative z-10">
                        <p className="text-[9px] font-black text-sky-500 uppercase tracking-widest mb-1">Targeted Assets</p>
                        <h4 className="text-2xl font-black text-white tabular-nums uppercase">{configs.filter(c => c.targetType !== 'all').length} Provisioned</h4>
                    </div>
                    <Layers className="text-sky-500/20 group-hover:scale-125 transition-transform" size={48} />
                </div>
                <div className="p-6 bg-black/80 rounded-[32px] border border-white/10 flex items-center justify-between group overflow-hidden">
                    <div className="relative z-10">
                        <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">Asset Library</p>
                        <h4 className="text-2xl font-black text-white tabular-nums uppercase">{media.length} Synchronized</h4>
                    </div>
                    <ImageIcon className="text-amber-500/20 group-hover:scale-125 transition-transform" size={48} />
                </div>
            </div>
          </div>
                                        ...
                                        <div className="flex items-end gap-4">
                                            <div className="flex-1 space-y-2">
                                                ...
                                                <select className="nexus-input bg-white" value={draft.content.url} onChange={(e) => setDraft({...draft, content: {...draft.content, url: e.target.value}})}>
                                                    ...
                                                    {media.filter(m => {
                                                        if (!m || !m.mimeType) return false;
                                                        return draft.contentType === 'image' ? m.mimeType.includes('image') : m.mimeType.includes('video');
                                                    }).map(m => (
                                                        <option key={m._id} value={m.path}>{m.originalName}</option>
                                                    ))}
                                                </select>
                                            </div>
                                            <div className="shrink-0">
                                                <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept={draft.contentType === 'image' ? "image/*" : "video/*"} />
                                                <button type="button" onClick={() => fileInputRef.current.click()} disabled={uploading} className="p-4 bg-white border border-slate-200 rounded-2xl text-sky-600 hover:bg-sky-50 transition-all shadow-sm">
                                    ...
                                    <div className="space-y-4 animate-fade-in">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                ...
                                                <input type="number" className="nexus-input py-2" value={draft.style.fontSize} onChange={(e) => setDraft({...draft, style: {...draft.style, fontSize: e.target.value}})}/>
                                            </div>
                                            <div className="space-y-1">
                ...
                <div className="h-full overflow-y-auto custom-scrollbar p-10">
                    {configs.length === 0 ? (
                        <div className="text-center py-40 border-2 border-dashed border-slate-200 rounded-[40px]">
                            <MonitorPlay className="text-slate-200 mx-auto mb-6" size={48} />
                            <p className="text-slate-400 uppercase font-black tracking-[8px] text-[10px]">No Active Protocols</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                            {configs.map(c => (
                                <div key={c._id} className="p-8 bg-white border border-slate-200 rounded-[48px] transition-all hover:border-emerald-300 hover:shadow-2xl hover:shadow-emerald-500/10 group relative flex flex-col">
                                    <div className="flex justify-between items-start mb-8">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border-2 transition-all ${c.isActive ? 'bg-emerald-50 border-emerald-100 text-emerald-500' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
                                            {c.contentType === 'image' ? <ImageIcon size={24}/> : c.contentType === 'video' ? <Video size={24}/> : c.contentType === 'text' ? <Type size={24}/> : <Palette size={24}/>}
                                        </div>
                                        <button onClick={() => deleteConfig(c._id)} className="p-3 bg-rose-50 text-rose-500 rounded-xl opacity-0 group-hover:opacity-100 hover:bg-rose-600 hover:text-white transition-all shadow-xl shadow-rose-500/10"><Trash2 size={16}/></button>
                                    </div>
                                    
                                    <div className="flex-1 space-y-2 mb-8">
                                        <div className="flex items-center gap-2">
                                            <div className={`w-2 h-2 rounded-full ${c.isActive ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[2px]">{c.contentType} Protocol</p>
                                        </div>
                                        <h3 className="text-xl font-black text-text uppercase tracking-tighter truncate leading-tight" title={c.name}>{c.name}</h3>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mb-8 border-t border-slate-100 pt-8">
                                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                                            ...
                                            <p className="text-[10px] font-black text-sky-600 uppercase truncate">{c.targetType}</p>
                                        </div>
                                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Priority</p>
                                            <p className="text-[10px] font-black text-emerald-600 tabular-nums uppercase">{c.priority}</p>
                                        </div>
                                    </div>

                                    <div className="p-5 bg-black/5 border border-black/5 rounded-2xl flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <Layers className="text-slate-400" size={14}/>
                                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{c.targetIds?.length || 0} Nodes Linked</span>
                                        </div>
                                        <span className="text-[8px] font-bold text-slate-300 tabular-nums">{new Date(c.createdAt).toLocaleDateString()}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
