...
const AppleDateInput = ({ value, onChange, label }) => {
    ...
    useEffect(() => {
        ...
        setLocalY(newY); setLocalM(newM); setLocalD(newD);
    }, [value]);
    const handleBlur = () => {
...
const BroadcastScheduler = ({ fetchData }) => {
  ...
  const [newSchedule, setNewSchedule] = useState({ 
    ...
    startTime: '09:00', endTime: '18:00', startDate: new Date().toISOString().slice(0, 10), endDate: new Date(Date.now() + 31536000000).toISOString().slice(0, 10),
  ...
  useEffect(() => {
    const fetchGroups = async () => { try { const res = await api.get('/api/groups'); setGroups(res.data); } catch (err) {} };
    fetchGroups();
  }, []);
  ...
  const createSchedule = async (e) => {
    ...
    try {
      ...
      setNewSchedule({ targetType: 'all', targetIds: [], templateId: '', mediaId: '', startTime: '09:00', endTime: '18:00', startDate: new Date().toISOString().slice(0, 10), endDate: new Date(Date.now() + 31536000000).toISOString().slice(0, 10), priority: 1, duration: 10 });
  ...
  const deleteSchedule = async (id) => {
    ...
    try { await api.delete(`/api/schedule/${id}`); toast.success('Terminated'); refetch(); if (fetchData) fetchData(); } catch (err) { toast.error('Failed'); }
  };
