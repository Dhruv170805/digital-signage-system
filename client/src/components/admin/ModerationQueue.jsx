...
import { CheckCircle, Eye, File, XCircle } from 'lucide-react';
...
const ModerationQueue = ({ pendingMedia, fetchData, setPreviewFile, setShowPreview }) => {
  const [modFile, setModFile] = useState(null);
  const [showModModal, setShowModModal] = useState(false);

  const handleModeration = async (id, action, data = {}) => {
    try {
      await api.post(`/api/media/${id}/${action}`, data);
