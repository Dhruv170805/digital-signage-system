  ...
  React.useEffect(() => {
    const fetchGroups = async () => { 
        try { 
            ...
            console.error('Failed to resolve cluster registry');
        } 
    };
    fetchGroups();
  }, []);
            ...
            <div className="lg:w-1/2 overflow-y-auto custom-scrollbar p-10 space-y-10 bg-white flex flex-col justify-center">
                <section>
                    <div className={`border-4 border-dashed rounded-[48px] p-16 text-center transition-all cursor-pointer group relative overflow-hidden flex flex-col justify-center min-h-[400px] ${file ? 'border-indigo-500 bg-indigo-50 shadow-2xl shadow-indigo-500/10' : 'border-slate-100 hover:border-indigo-300 hover:bg-slate-50'}`} onClick={() => document.getElementById('media-upload').click()}>
                        ...
                        {file ? (<div className="space-y-2"><p className="text-2xl font-black text-slate-900 truncate max-w-xs mx-auto uppercase tracking-tighter">{file.name}</p><p className="text-indigo-600 text-[10px] font-black uppercase tracking-[4px]">{(file.size / 1024 / 1024).toFixed(2)} MB • READY</p></div>) : (<div className="space-y-2"><p className="text-xl font-black text-slate-900 uppercase tracking-tighter">Select File</p><p className="text-slate-400 text-[10px] font-black uppercase tracking-[2px]">PDF, MP4, WEBM, JPG, PNG</p></div>)}
                    </div>
                </section>
            </div>
            <div className="lg:w-1/2 overflow-y-auto custom-scrollbar p-8 bg-slate-50/50">
                <section>
                    <h4 className="text-[10px] font-black uppercase text-sky-600 mb-6 flex items-center gap-3"><div className="w-6 h-px bg-sky-600/30" /> Upload Settings</h4>
                    <form onSubmit={handleUpload} className="space-y-6 p-8 bg-white border border-slate-200 rounded-[40px] shadow-sm">
                        <div className="space-y-6">
                            <div className="space-y-4"><h5 className="text-[10px] font-black uppercase text-indigo-600 ml-1 tracking-[2px]">Screen Selection</h5><div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-[8px] font-black uppercase text-slate-400 ml-1">Select group</label><select className="nexus-input bg-slate-50 border-slate-200 text-[10px] h-10" value={requestedTargetType} onChange={(e) => setRequestedTargetType(e.target.value)}><option value="all">All Screens</option><option value="group">Group</option><option value="screen">Specific Screen</option></select></div><div className="space-y-1.5"><label className="text-[8px] font-black uppercase text-slate-400 ml-1">Select Screen</label><select disabled={requestedTargetType === 'all'} className="nexus-input bg-slate-50 border-slate-200 text-[10px] h-10 disabled:opacity-20" value={requestedTargetId} onChange={(e) => setRequestedTargetId(e.target.value)}><option value="">Choose Screen/Group...</option>{requestedTargetType === 'screen' ? screens.map(s => <option key={s._id} value={s._id}>{s.name}</option>) : groups.map(g => <option key={g._id} value={g._id}>{g.name}</option>)}</select></div></div></div>
                            <div className="space-y-3 pt-4 border-t border-slate-100">
                                <h5 className="text-[10px] font-black uppercase text-amber-600 ml-1 tracking-[2px]">Time Settings</h5>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5"><label className="text-[8px] font-black uppercase text-slate-400 ml-1">From Date</label><input type="date" required className="nexus-input bg-slate-50 border-slate-200 text-[10px] h-10" value={requestedStartDate} onChange={(e) => setRequestedStartDate(e.target.value)}/></div>
                                    <AppleTimeInput label="From Time" value={requestedStartTime} onChange={setRequestedStartTime} />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5"><label className="text-[8px] font-black uppercase text-slate-400 ml-1">To Date</label><input type="date" required className="nexus-input bg-slate-50 border-slate-200 text-[10px] h-10" value={requestedEndDate} onChange={(e) => setRequestedEndDate(e.target.value)}/></div>
                                    <AppleTimeInput label="To Time" value={requestedEndTime} onChange={setRequestedEndTime} />
                                </div>
                                <div className="grid grid-cols-2 gap-4 pt-2">
                                    <div className="space-y-1.5">
                                        <label className="text-[8px] font-black uppercase text-slate-400 ml-1">Priority</label>
                                        <select 
                                           className="nexus-input bg-slate-50 border-slate-200 text-[10px] h-10 font-black uppercase" 
                                           value={requestedPriority} 
                                           onChange={(e) => setRequestedPriority(Number(e.target.value))}
                                        >
                                    ...
                                    <div className="space-y-1.5"><label className="text-[8px] font-black uppercase text-slate-400 ml-1">Duration (Sec)</label><input type="number" min="1" required className="nexus-input bg-slate-50 border-slate-200 text-[10px] h-10" value={requestedDuration} onChange={(e) => setRequestedDuration(Number(e.target.value))}/></div>
                                </div>                            </div>
                        </div>
                        <button type="submit" disabled={!file || (requestedTargetType !== 'all' && !requestedTargetId) || uploading} className="nexus-btn-primary w-full py-5 flex items-center justify-center gap-4 shadow-xl shadow-indigo-500/10"><span className="text-[9px] font-black uppercase tracking-[4px]">Request</span><Send className="w-4 h-4" /></button>
                    </form>
                </section>
            </div>
          </div>
      ...
      case 'history':
        return (
            <div className="h-full overflow-y-auto custom-scrollbar p-10 bg-slate-50/30 space-y-10">
                <section>
                    <h4 className="text-[10px] font-black uppercase text-indigo-600 mb-8 flex items-center gap-3"><div className="w-6 h-px bg-indigo-600/30" /> My Files Status</h4>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <StatWidget label="Total Files" value={stats.total} icon={Layers} onClick={() => setRegistryFilter('all')} isActive={registryFilter === 'all'} />
                        <StatWidget label="Approved" value={stats.active} icon={CheckCircle} color="emerald" onClick={() => setRegistryFilter('approved')} isActive={registryFilter === 'approved'} />
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[4px]">File History</h4>
                ...
                {filteredMyFiles.length === 0 ? (<div className="text-center py-40 border-2 border-dashed border-slate-200 rounded-[60px] bg-white"><HistoryIcon className="w-20 h-20 mx-auto mb-6 text-slate-200" /><p className="font-black uppercase tracking-[8px] text-[10px] text-slate-400">No Files Found</p></div>) : (
                <div className="bg-white border border-slate-200 rounded-[40px] overflow-hidden shadow-sm">
                    <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200"><tr className="text-[9px] font-black uppercase text-slate-400 tracking-widest"><th className="py-6 px-10">File Name</th><th className="py-6 px-10">Screen Name</th><th className="py-6 px-10">Time Window</th><th className="py-6 px-10">Status</th><th className="py-6 px-10 text-right">Ops</th></tr></thead>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                                                    {f.requestedTargetId ? (screens.find(s => s._id === f.requestedTargetId)?.name || groups.find(g => g._id === f.requestedTargetId)?.name || 'CONNECTED SCREEN') : 'ALL SCREENS'}
                                                </p>
                                            </div>
                                        </div>
                                    </td>
  ...
  return (
    <Shell role="user" activeTab={activeTab} setActiveTab={setActiveTab}>
      <div className="h-full flex flex-col">
          <div className="bg-white p-8 border-b border-slate-200 shrink-0"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6"><div><div className="flex items-center gap-3 mb-2"><Activity className="text-indigo-600" size={16} /><span className="text-[10px] font-black uppercase tracking-[4px] text-indigo-600">Request Center</span></div><h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">My Requests</h2></div><div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl border border-slate-200"><button onClick={() => setActiveTab('upload')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'upload' ? 'bg-white text-black shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}>Upload Files</button><button onClick={() => setActiveTab('history')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-white text-black shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}>History</button><button onClick={() => setActiveTab('live')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'live' ? 'bg-white text-black shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}>Screen Preview</button></div></div></div>
