            ...
            <div className="lg:w-1/2 overflow-y-auto custom-scrollbar p-8 bg-slate-50/50">
                <section>
                    ...
                    <form onSubmit={handleUpload} className="space-y-6 p-8 bg-white border border-slate-200 rounded-[40px] shadow-sm">
                        ...
                        <button type="submit" disabled={!file || (requestedTargetType !== 'all' && !requestedTargetId) || uploading} className="nexus-btn-primary w-full py-5 flex items-center justify-center gap-4 shadow-xl shadow-indigo-500/10"><span className="text-[9px] font-black uppercase tracking-[4px]">START BROADCAST</span><Send className="w-4 h-4" /></button>
                    </form>
                </section>
            </div>
          </div>
  ...
  return (
    <Shell role="user" activeTab={activeTab} setActiveTab={setActiveTab}>
      <div className="h-full flex flex-col">
          <div className="bg-white p-8 border-b border-slate-200 shrink-0"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6"><div><div className="flex items-center gap-3 mb-2"><Activity className="text-indigo-600" size={16} /><span className="text-[10px] font-black uppercase tracking-[4px] text-indigo-600">User Dashboard</span></div><h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">My Dashboard</h2></div><div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl border border-slate-200"><button onClick={() => setActiveTab('upload')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'upload' ? 'bg-white text-black shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}>Upload Files</button><button onClick={() => setActiveTab('history')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-white text-black shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}>History</button><button onClick={() => setActiveTab('live')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'live' ? 'bg-white text-black shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}>Screen Preview</button></div></div></div>
